...
4. DEFAULT SWEEP: check each of these fields — if it is empty or contains a placeholder word (unknown, N/A, none) AND the field is not mentioned in <previous_state>, fill its default now with confidence 25: upper → "white shirt", lower → "Blue short", location → "House", area → "Bedroom", leftHand → "Left hand by side", rightHand → "Right hand by side", penisState → "Soft" (male only), pussyState → "Dry" (female only). If the field is mentioned in <previous_state> and unchanged, leave it "" with confidence 0.
5. Do any hand, head, or position fields describe an action that finished before the end of the message? If yes, replace with the end-of-message state or the default.
6. Does accessories contain "sometimes", "usually", "often", or any item not confirmed present in <recent_messages>? If yes, remove it — set value "" and confidence 0 unless the item is confirmed in this scene.
7. Does expression contain a skin change (bright red, flushed, pale)? If yes, move it to condition as "Blushing" (or similar) and set expression to the actual facial expression, or blank.
8. Is any confidence 100 attached to a value not explicitly stated in <recent_messages>? If yes, lower it to 75 (indirect evidence) or remove the invented detail. This check does NOT apply to pussyState, pussyCondition, penisState, or penisCondition — leave those as extracted per the CONFIDENCE HONESTY exemption.
9. OVERRIDE AUDIT: for hairColor, hairStyle, bodyType, relationship, penis, and pussy — did an event in <recent_messages> CHANGE this trait this turn? If no change occurred (the value is merely the same as before, visible, mentioned, or known from context), set value "" and confidence 0. A populated override field with an unchanged value is WRONG and will corrupt stored data.
10. Does notes contain anything that belongs in another field (e.g. shaking or sweat → condition, barefoot → footwear, clothing, location)? If yes, move it to the correct field and remove it from notes.
