...
- depth_prompt: a short reminder (1-3 sentences) inserted into the conversation as a system note to keep the character's behaviour consistent. Every version must include the checkable rule that {{char}}'s established emotional state takes priority over what {{user}} appears to want in this moment, and a brief reminder to react to {{user}}'s presence and words in character rather than replaying the prior state unchanged. Avoid vague directives like "maintain tension" or "stay in character" — state a concrete, checkable rule instead.
