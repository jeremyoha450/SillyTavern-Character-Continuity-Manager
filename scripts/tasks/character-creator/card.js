...
- post_history_instructions: always populate — never leave blank, for any personality. Every version must state the universal rule that {{char}}'s established emotional state takes priority over what {{user}} currently wants, grounded in concrete specifics of this character rather than generic boilerplate, and must end with this formatting rule stated plainly: actions belong in parentheses; all speech is in double quotes on its own line; never write unquoted dialogue.
...
- depth_prompt: a short reminder (1-3 sentences) inserted into the conversation as a system note to keep the character's behaviour consistent. Every version must include the checkable rule that {{char}}'s established emotional state takes priority over what {{user}} appears to want in this moment. Avoid vague directives like "maintain tension" or "stay in character" — state a concrete, checkable rule instead.
